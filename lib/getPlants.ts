export async function getAllPlants(term: string) {
  ...
  const normalizedData1 = dataPerenual.data.map((item: any) => {
    ...
    const normalizedItem = {
      ...
      watering,
      sunlight,
      image: validImageUrl || "There is no Image", // Use null if no valid URL is found
    };
  ...
  const normalizedData2 = dataTrefle.data.map((item: any) => {
    return {
      ...
      image: item?.image_url || "There is no Image",
    };
  });

  // Combine the normalized data
  const combinedData = [...normalizedData1];

  // Now combinedData will have the format you specified
  return combinedData;
}

export async function getTreflePlants(term: string) {
  const resTrefle = await fetch(
    `https://trefle.io/api/v1/plants/search?token=q5kBFf-5sK8dEWvwRJRH7I9dRrWgE3vpK47L2ZENrYw&q=coconut`
  );
  ...
  // // Combine the normalized data
  // const combinedData = [...normalizedData2];

  // Now combinedData will have the format you specified
  return dataTrefle.data;
}
